...
/*************************************************************
  NAME         : ModifyInstance
  DESCRIPTION  : Modifies c_slots of an instance via the
...
/**************************************************************
  ...
  DESCRIPTION  : Modifies c_slots of an instance of a class
...
/**********************************************************
  ...
  RETURNS      : Nothing useful
  SIDE EFFECTS : c_slots updated (messages sent)
  NOTES        : None
 **********************************************************/
...
static void DuplicateMsgHandlerSupport(
  ...
  int msgpass)
  {
   ...
   for (i = 0 ; i < dstins->cls->localInstanceSlotCount ; i++)
     {
      if (dstins->c_slots[i].override == FALSE)
        {
         if (msgpass)
           {
            temp.type = (unsigned short)  srcins->c_slots[i].type;
            temp.value = srcins->c_slots[i].value;
            ...
            DirectMessage(theEnv,dstins->c_slots[i].desc->overrideMessage,
                          dstins,&temp,valArg);
         ...
         else
           {
            temp.type = (unsigned short) srcins->c_slots[i].type;
            temp.value = srcins->c_slots[i].value;
            if (srcins->c_slots[i].type == MULTIFIELD)
              {
               SetDOBegin(temp,1);
               SetDOEnd(temp,GetMFLength(srcins->c_slots[i].value));
              }
            if (PutSlotValue(theEnv,dstins,&dstins->c_slots[i],&temp,&junk,"duplicate-instance")
                 == FALSE)
              goto DuplicateError;
           }
        }
