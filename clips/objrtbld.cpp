...
static intBool CheckDuplicatec_slots(void *,struct lhsParseNode *,SYMBOL_HN *);
   ...
   while (theToken.type != RPAREN)
     {
      ...
      if (CheckDuplicatec_slots(theEnv,firstNode,(SYMBOL_HN *) theToken.value))
        goto ObjectLHSParseERROR;
...
/**************************************************************
  ...
  DESCRIPTION  : This function reexamines the object pattern
                 ...
                 has been propagated from other patterns.
                   Any c_slots which are no longer applicable
...
static intBool ReorderAndAnalyzeObjectPattern(
  ...
  struct lhsParseNode *topNode)
  {
   ...
   /* ================================================================
      For each of the c_slots (excluding name and is-a), check the total
...
/************************************************************************
  ...
  INPUTS       : 1) The current layer of nodes being examined in the
                 ...
                 3) A buffer for holding the first node of a group
                    of c_slots with the same name as the new node
...
static OBJECT_PATTERN_NODE *CreateNewObjectPatternNode(
  ...
  unsigned constantSelector)
  {
   ...
   /* ===========================================================
      ...
      with constant tests on a particular c_slots.  If we put
...
/******************************************************
  ...
  NOTES        : None
 ******************************************************/
static intBool CheckDuplicatec_slots(
  void *theEnv,
...
/***************************************************
  ...
  INPUTS       : 1) The logical input source
                 ...
                 3) Constraint record holding the
                    ...
                    c_slots which could match the
                    slot pattern
                 4) A flag indicating if any
                    multifield c_slots match the name
  RETURNS      : The intermediate pattern nodes
...
/****************************************************************
  ...
  DESCRIPTION  : Determines which c_slots could match the slot
  ...
  INPUTS       : 1) The class set
                 ...
                 3) A buffer to hold a flag indicating if
                    any multifield c_slots are found w/ this name
  RETURNS      : A union of the constraints on all the c_slots
                 which could match the c_slots (NULL if no
                 c_slots found)
...
/*****************************************************************
  ...
  SIDE EFFECTS : clsset is attached to extra slot pattern
                 Pointers to the is-a and name c_slots are also
...
static struct lhsParseNode *FilterObjectPattern(
  ...
  struct lhsParseNode *unfilteredc_slots,
   ...
   *bitmap_slot = GetLHSParseNode(theEnv);
   ...
   (*bitmap_slot)->userData = unfilteredc_slots->userData;
   unfilteredc_slots->userData = NULL;

   /* ========================
      Find is-a and name nodes
      ======================== */
   prv = NULL;
   cur = unfilteredc_slots;
   while (cur != NULL)
     {
   ...
   /* ================================
      ...
      ================================ */
   if (prv == NULL)
     unfilteredc_slots = *bitmap_slot;
   else
     prv->right = *bitmap_slot;
   return(unfilteredc_slots);
  }
...
/***************************************************
  ...
  DESCRIPTION  : Examines an object pattern and
                 ...
                 the ids of the c_slots used in
                 the pattern
  ...
  SIDE EFFECTS : Bitmap created and added to hash
                 ...
                 for ids of c_slots used in pattern
  NOTES        : None
 ***************************************************/
...
static struct lhsParseNode *RemoveSlotExistenceTests(
  ...
  BITMAP_HN **bmp)
  {
   ...
   while (tempPattern != NULL)
     {
      ...
      /* =====================================================
         If none of the other tests for removing c_slots or slot
   ...
   /* ====================================
      Return the pattern with unused c_slots
