...
static void BuildDefaultc_slots(void *,intBool);
...
globle INSTANCE_TYPE *BuildInstance(
  ...
  intBool initMessage)
  {
   ...
   BuildDefaultc_slots(theEnv,initMessage);
...
/*****************************************************************************
  ...
  SIDE EFFECTS : Each DATA_OBJECT slot in the instance's slot array is replaced
                   ...
                   are deleted.
  NOTES        : H/L Syntax: (init-c_slots <instance>)
 *****************************************************************************/
globle void Initc_slotsCommand(
  void *theEnv,
  DATA_OBJECT *result)
  {
   SetpType(result,SYMBOL);
   ...
   if (CheckCurrentMessage(theEnv,"init-c_slots",TRUE) == FALSE)
     return;
...
globle intBool QuashInstance(
  ...
  INSTANCE_TYPE *ins)
  {
   ...
   /* ==============================================
      ...
      rule, don't bother deleting its c_slots yet, for
 ...
 ****************************************************************/
static void BuildDefaultc_slots(
  void *theEnv,
  intBool initMessage)
  {
   ...
   if (scnt > 0)
     {
      ...
      if (lscnt != 0)
        InstanceData(theEnv)->CurrentInstance->c_slots = dst =
           (INSTANCE_SLOT *) gm2(theEnv,(sizeof(INSTANCE_SLOT) * lscnt));
      src = InstanceData(theEnv)->CurrentInstance->cls->instanceTemplate;

      /* ==================================================
         A map of slot addresses is created - shared c_slots
         point at values in the class, and local c_slots
         point at values in the instance

         Also - c_slots are always given an initial value
         (since c_slots cannot be unbound). If there is
...
/*******************************************************************
  ...
  SIDE EFFECTS : EvaluationError set on errors - c_slots evaluated
  NOTES        : None
 *******************************************************************/
static int CoreInitializeInstance(
  ...
  EXPRESSION *ovrexp)
  {
   ...
   /* =================================================================
      If the c_slots are initialized properly - the initializeInProgress
      flag will be turned off.
      ================================================================= */
   ins->initializeInProgress = 1;
   ins->initc_slotsCalled = 0;
   ...
   return((ins->initc_slotsCalled == 0) ? FALSE : TRUE);
  }
...
/*****************************************************************************
  ...
  DESCRIPTION  : Evaluates default c_slots only - c_slots that were specified
                 by overrides (sp->override == 1) are ignored)
  INPUTS       : 1) Instance address
  RETURNS      : Nothing useful
  SIDE EFFECTS : Each DATA_OBJECT slot in the instance's slot array is replaced
   ...
   if (ins->initializeInProgress == 0)
     {
      ...
      SetEvaluationError(theEnv,TRUE);
      EnvPrintRouter(theEnv,WERROR,"init-c_slots not valid in this context.\n");
      return;
     }
   for (i = 0 ; i < ins->cls->instanceSlotCount ; i++)
     {
      ...
      /* ===========================================================
         Slot-overrides are just a short-hand for put-c_slots, so they
         ...
         if (slot->desc->dynamicDefault)
           {
            if (EvaluateAndStoreInDataObject(theEnv,(int) slot->desc->multiple,
                                             ...
                                             &temp,TRUE))
              PutSlotValue(theEnv,ins,slot,&temp,&junk,"function init-c_slots");
           }
