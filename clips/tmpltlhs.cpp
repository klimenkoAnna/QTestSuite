...
/***************************************/
   ...
   static struct lhsParseNode    *GetLHSc_slots(void *,char *,struct token *,struct deftemplate *,int *);
   static struct lhsParseNode    *GetSingleLHSSlot(void *,char *,struct token *,
                                                   struct templateSlot *,int *,short);
   static intBool                 MultiplyDefinedLHSc_slots(void *,struct lhsParseNode *,SYMBOL_HN *);
...
globle struct lhsParseNode *DeftemplateLHSParse(
  ...
  struct deftemplate *theDeftemplate)
  {
   ...
   error = FALSE;
   firstSlot = GetLHSc_slots(theEnv,readSource,&theToken,theDeftemplate,&error);
   if (error)
     {
...
/******************************************/
/* GetLHSc_slots: Retrieves all of the slot */
/*   values used in a LHS pattern.        */
/******************************************/
static struct lhsParseNode *GetLHSc_slots(
  void *theEnv,
  char *readSource,
   ...
   while (tempToken->type != RPAREN)
     {
      ...
      if (MultiplyDefinedLHSc_slots(theEnv,firstSlot,(SYMBOL_HN *) tempToken->value) == TRUE)
        {
         *error = TRUE;
         ReturnLHSParseNodes(theEnv,firstSlot);
         return(NULL);
...
/******************************************************/
/* MultiplyDefinedLHSc_slots: Determines if a slot name */
/*   was used more than once in a LHS pattern.        */
/******************************************************/
static intBool MultiplyDefinedLHSc_slots(
  void *theEnv,
  struct lhsParseNode *thec_slots,
  SYMBOL_HN *slotName)
  {
   for (;
        thec_slots != NULL;
        thec_slots = thec_slots->right)
     {
      if (thec_slots->slot == slotName)
        {
