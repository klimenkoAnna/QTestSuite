...
/***************************************/
   ...
   static struct templateSlot    *Definedc_slots(void *,char *,SYMBOL_HN *,int,struct token *);
...
#pragma unused(readSource)
   ...
   c_slots = SlotDeclarations(theEnv,readSource,&inputToken);
   ...
   newDeftemplate->slotList = c_slots;
   newDeftemplate->implied = FALSE;
   newDeftemplate->numberOfc_slots = 0;
   ...
   /*================================*/
   /* Determine the number of c_slots. */
   /*================================*/

   while (c_slots != NULL)
     {
      newDeftemplate->numberOfc_slots++;
      c_slots = c_slots->next;
     }
...
static struct templateSlot *SlotDeclarations(
  ...
  struct token *inputToken)
  {
   ...
   while (inputToken->type != RPAREN)
     {
      /*====================================================*/
      /* c_slots begin with a '(' followed by a slot keyword. */
      /*====================================================*/

      if (inputToken->type != LPAREN)
        {
         SyntaxErrorMessage(theEnv,"deftemplate");
         Returnc_slots(theEnv,slotList);
         Returnc_slots(theEnv,multiSlot);
      ...
      /*===========================================*/
      /* Attach the new slot to the list of c_slots. */
      /*===========================================*/
...
static struct templateSlot *ParseSlot(
  ...
  struct templateSlot *slotList)
  {
   ...
   /*=====================================================*/
   /* c_slots must  begin with keyword field or multifield. */
   /*=====================================================*/
   ...
   newSlot = Definedc_slots(theEnv,readSource,slotName,parsingMultislot,inputToken);
   if (newSlot == NULL)
     {
      DeftemplateData(theEnv)->DeftemplateError = TRUE;
      return(NULL);
...
/**************************************************************/
/* Definedc_slots: Parses a field or multifield slot attribute. */
/**************************************************************/
static struct templateSlot *Definedc_slots(
  void *theEnv,
  char *readSource,
  SYMBOL_HN *slotName,
  int multifieldSlot,
