...
static void MarkDefclassAndc_slots(void *,struct constructHeader *,void *);
...
static intBool c_slotsToCode(void *,FILE **,char *,int,int,FILE *,
                           int *,int,DEFCLASS *,int *,
                           int *,int *,struct CodeGeneratorFile *);
static intBool Templatec_slotsToCode(void *,FILE **,char *,int,int,FILE *,
                                   int *,int,DEFCLASS *,int *,
                                   int *,int *,struct CodeGeneratorFile *);
static intBool Orderedc_slotsToCode(void *,FILE **,char *,int,int,FILE *,
                                  int *,int,DEFCLASS *,int *,
                                  int *,int *,struct CodeGeneratorFile *);
...
static void ReadyObjectsForCode(
  void *theEnv)
  {
   ...
   DoForAllConstructs(theEnv,MarkDefclassAndc_slots,DefclassData(theEnv)->DefclassModuleIndex,
                      FALSE,(void *) &markInfo);
   i = 0L;
...
/************************************************************
  ...
  INPUTS       : 1) The defclass
                 2) A buffer containing the info:
                    ...
                    b) The current partition # for c_slots
                    c) The current offset in that partition
                    d) The max # of elements in any array
  RETURNS      : Nothing useful
  SIDE EFFECTS : Bsave indices of classes and c_slots set
  NOTES        : The template c_slots are written at the
                 same time as the real c_slots - thus the
                 references must be predetermined
 ************************************************************/
#if WIN_BTC
#pragma argsused
#endif
static void MarkDefclassAndc_slots(
  void *theEnv,
   ...
   for (i = 0 ; i < theDefclass->slotCount ; i++)
     {
      ...
      theDefclass->c_slots[i].bsaveIndex = theLocationInfo.theLong;
      markInfo->slotCount++;
   ...
   while (theModule != NULL)
     {
      ...
      for (theDefclass = (DEFCLASS *) EnvGetNextDefclass(theEnv,NULL) ;
           ...
           theDefclass = (DEFCLASS *) EnvGetNextDefclass(theEnv,(void *) theDefclass))
        {
         ...
         if (c_slotsToCode(theEnv,&itemFiles[SLOTI],fileName,fileID,imageID,
                         headerFP,&fileCount,maxIndices,theDefclass,
                         &itemArrayVersions[SLOTI],&itemArrayCounts[SLOTI],
         ...
         if (Templatec_slotsToCode(theEnv,&itemFiles[TSLOTI],fileName,fileID,imageID,
                                 headerFP,&fileCount,maxIndices,theDefclass,
                                 &itemArrayVersions[TSLOTI],&itemArrayCounts[TSLOTI],
         ...
         if (Orderedc_slotsToCode(theEnv,&itemFiles[OSLOTI],fileName,fileID,imageID,
                                headerFP,&fileCount,maxIndices,theDefclass,
                                &itemArrayVersions[OSLOTI],&itemArrayCounts[OSLOTI],
...
/****************************************************************
  ...
  INPUTS       : 1)  The output file
                 ...
                 9)  The relative index of the
                     c_slots in the partition
                 10) The partition holding the
                     defclass template c_slots
                 11) The relative index of the
                     template c_slots in the partition
...
/***********************************************************
  NAME         : c_slotsToCode
  DESCRIPTION  : Prints out c_slots - all c_slots
                 for a particular class are
                 guaranteed to be in the same
                 array partition
  INPUTS       : 1)  A buffer for the c_slots file
                 2)  The base image name
                 3)  The id for this type of data
                 4)  The base image id
                 5)  The general header file
                 ...
                 7)  The maximum # of elements in any array
                 8)  A pointer to the class
                 9)  A buffer holding the c_slots partition #
                 10) A buffer holding the c_slots relative
                     index in the partition
                 11) A buffer for a flag indicating if the
                     buffer file can be reopened later
                 12) A pointer to the file info for
  ...
  RETURNS      : TRUE if all OK, FALSE
                 otherwise
  SIDE EFFECTS : c_slots written
  NOTES        : None
 ***********************************************************/
static intBool c_slotsToCode(
  void *theEnv,
  FILE **slotFile,
  char *fileName,
  int fileID,
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static intBool Templatec_slotsToCode(
  void *theEnv,
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static intBool Orderedc_slotsToCode(
  void *theEnv,
