...
/***********************************************************
  ...
  DESCRIPTION  : Examines arguments for:
                   class-c_slots, get-defmessage-handler-list,
...
/********************************************************************
  NAME         : Classc_slotsCommand
  DESCRIPTION  : Groups slot info for a class into a multifield value
                   for dynamic perusal
  INPUTS       : Data object buffer to hold the c_slots of the class
  RETURNS      : Nothing useful
  SIDE EFFECTS : Creates a multifield storing the names of
                    the c_slots of the class
  NOTES        : Syntax: (class-c_slots <class> [inherit])
 ********************************************************************/
globle void Classc_slotsCommand(
  void *theEnv,
  DATA_OBJECT *result)
  {
   int inhp;
   ...
   clsptr = ClassInfoFnxArgs(theEnv,"class-c_slots",&inhp);
   if (clsptr == NULL)
     {
      EnvSetMultifieldErrorValue(theEnv,result);
      return;
     }
   EnvClassc_slots(theEnv,clsptr,result,inhp);
  }
...
globle void c_slotsourcesCommand(
  void *theEnv,
  DATA_OBJECT *result)
  {   
   SlotInfoSupportFunction(theEnv,result,"slot-sources",Envc_slotsources);
  }
...
/********************************************************************
  NAME         : EnvClassc_slots
  DESCRIPTION  : Groups slot info for a class into a multifield value
                   for dynamic perusal
  INPUTS       : 1) Generic pointer to class
                 2) Data object buffer to hold the c_slots of the class
                 3) Include (1) or exclude (0) inherited c_slots
  RETURNS      : Nothing useful
  SIDE EFFECTS : Creates a multifield storing the names of
                    the c_slots of the class
  NOTES        : None
 ********************************************************************/
globle void EnvClassc_slots(
  void *theEnv,
  void *clsptr,
  DATA_OBJECT *result,
  int inhp)
   ...
   if (inhp)
   ...
   else
     {
      for (i = 0 ; i < cls->slotCount ; i++)
        {
         SetMFType(result->value,i+1,SYMBOL);
         SetMFValue(result->value,i+1,cls->c_slots[i].slotName->name);
        }
     }
  }
