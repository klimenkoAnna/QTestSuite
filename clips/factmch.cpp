...
static void PatternNetErrorMessage(
  ...
  {
   char buffer[60];
   struct templateSlot *thec_slots;
   int i;
   ...
   if (FactData(theEnv)->CurrentPatternFact->whichDeftemplate->implied)
     ...
     {
      thec_slots = FactData(theEnv)->CurrentPatternFact->whichDeftemplate->slotList;
      for (i = 0; i < (int) patternPtr->whichSlot; i++) thec_slots = thec_slots->next;
      gensprintf(buffer,"   Problem resides in slot %s\n",ValueToString(thec_slots->slotName));
     }
