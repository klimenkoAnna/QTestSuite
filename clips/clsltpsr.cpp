...
/***************************************************
  NAME         : Deletec_slots
  DESCRIPTION  : Deallocates a list of c_slots and
                   their values
...
/**********************************************************
  NAME         : InsertSlot
  DESCRIPTION  : Inserts a slot into the list of c_slots
      ...
      while (stmp != NULL)
        {
         if (stmp->desc->slotName == slot->slotName)
           {
            tmp->nxt = slist;
            Deletec_slots(theEnv,tmp);
            PrintErrorID(theEnv,"CLSLTPSR",1,FALSE);
            EnvPrintRouter(theEnv,WERROR,"Duplicate c_slots not allowed.\n");
            return(NULL);
           }
         sprv = stmp;
         stmp = stmp->nxt;
...
/**************************************************************************
  ...
  DESCRIPTION  : Composite c_slots are ones that get their facets
      ...
      if (parsedConstraint->cardinality)
        {
         ...
         EnvPrintRouter(theEnv,WERROR,"Cardinality facet can only be used with multifield c_slots\n");
         return(FALSE);
        }
      else
        {
   ...
   if (sd->noDefault && sd->noWrite)
     {
      ...
      EnvPrintRouter(theEnv,WERROR,"read-only c_slots must have a default value\n");
      return(FALSE);
     }
   if (sd->noWrite && (sd->createWriteAccessor || sd->overrideMessageSpecified))
     {
      PrintErrorID(theEnv,"CLSLTPSR",5,TRUE);
      EnvPrintRouter(theEnv,WERROR,"read-only c_slots cannot have a write accessor\n");
      return(FALSE);
     }
   if (sd->noInherit && sd->publicVisibility)
     {
      PrintErrorID(theEnv,"CLSLTPSR",6,TRUE);
      EnvPrintRouter(theEnv,WERROR,"no-inherit c_slots cannot also be public\n");
      return(FALSE);
     }
   return(TRUE);
  }
...
/********************************************************************
  ...
  NOTES        : On errors, slot is marked as dynamix so that
                 Deletec_slots() will erase the slot expression
 ********************************************************************/
static intBool EvaluateSlotDefaultValue(
  ...
  char *specbits)
  {
   ...
   /* ===================================================================
      ...
      that Deletec_slots() would erase in the event of an error.  The delay
