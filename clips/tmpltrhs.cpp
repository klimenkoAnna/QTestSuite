   ...
   while ((slotPtr = ParseSlotLabel(theEnv,readSource,theToken,theDeftemplate,error,endType)) != NULL)
     {
      ...
      /*===================================================*/
      /* Add the slot to the list of c_slots already parsed. */
      /*===================================================*/
...
static struct expr *ReorderAssertSlotValues(
  ...
  int *error)
  {
   ...
   /*=============================================*/
   /* Loop through each of the c_slots in the order */
...
/*******************************************************************/
/* FindAssertSlotItem: Finds a particular slot in a list of c_slots. */
/*******************************************************************/
static struct expr *FindAssertSlotItem(
  struct templateSlot *slotPtr,
  struct expr *listOfc_slots)
  {
   while (listOfc_slots != NULL)
     {
      if (listOfc_slots->value == (void *) slotPtr->slotName) return (listOfc_slots);
      listOfc_slots = listOfc_slots->nextArg;
     }
