...
/*            FALSE for c_slots having initialize-only access.  */
/*            DR0860                                          */
/*                                                            */
...
static intBool Printc_slotsources(void *,char *,SYMBOL_HN *,PACKED_CLASS_LINKS *,long,int);
   ...
   if (cls->instanceTemplate != NULL)
     {
      ...
      EnvPrintRouter(theEnv,logicalName,"\nConstraint information for c_slots:\n\n");
...
static SLOT_DESC *LookupSlot(
  ...
  intBool inheritFlag)
  {
   SYMBOL_HN *c_slotsymbol;
   int slotIndex;
   SLOT_DESC *sd;

   c_slotsymbol = FindSymbolHN(theEnv,slotName);
   if (c_slotsymbol == NULL)
     return(NULL);
   slotIndex = FindInstanceTemplateSlot(theEnv,theDefclass,c_slotsymbol);
   if (slotIndex == -1)
     return(NULL);
...
/*************************************************************
  ...
  DESCRIPTION  : Displays a table summary of basic
                  facets for the c_slots of a class
                  including:
                  single/multiple
...
static void DisplaySlotBasicInfo(
  ...
  DEFCLASS *cls)
  {
   ...
   gensprintf(buf,slotNamePrintFormat,"c_slots");
#if DEFRULE_CONSTRUCT
   ...
   for (i = 0 ; i < cls->instanceSlotCount ; i++)
     {
      ...
      EnvPrintRouter(theEnv,logicalName,buf);
      Printc_slotsources(theEnv,logicalName,sp->slotName->name,&sp->cls->allSuperclasses,0,TRUE);
      EnvPrintRouter(theEnv,logicalName,"\n");
     }
  }

/***************************************************
  NAME         : Printc_slotsources
  ...
  NOTES        : None
 ***************************************************/
static intBool Printc_slotsources(
  void *theEnv,
  char *logicalName,
  SYMBOL_HN *sname,
   ...
   if ((csp != NULL) ? ((csp->noInherit == 0) || inhp) : FALSE)
     {
      if (csp->composite)
        {
         if (Printc_slotsources(theEnv,logicalName,sname,sprec,theIndex+1,FALSE))
           EnvPrintRouter(theEnv,logicalName," ");
        }
     ...
     return(Printc_slotsources(theEnv,logicalName,sname,sprec,theIndex+1,FALSE));
  }
