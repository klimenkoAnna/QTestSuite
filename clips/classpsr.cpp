...
static TEMP_SLOT_LINK *Mergec_slots(void *,TEMP_SLOT_LINK *,DEFCLASS *,short *,int);
static void Packc_slots(void *,DEFCLASS *,TEMP_SLOT_LINK *);
...
globle int ParseDefclass(
  ...
  char *readSource)
  {
   ...
   while (GetType(DefclassData(theEnv)->ObjectParseToken) != RPAREN)
     {
      ...
      if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),ROLE_RLN) == 0)
        ...
        {
         c_slots = ParseSlot(theEnv,readSource,c_slots,preclist,FALSE,FALSE);
         if (c_slots == NULL)
           {
            parseError = TRUE;
            break;
        ...
        {
         c_slots = ParseSlot(theEnv,readSource,c_slots,preclist,FALSE,TRUE);
         if (c_slots == NULL)
           {
            parseError = TRUE;
            break;
        ...
        {
         c_slots = ParseSlot(theEnv,readSource,c_slots,preclist,TRUE,TRUE);
         if (c_slots == NULL)
           {
            parseError = TRUE;
            break;
   ...
   /* =================================
      Shove c_slots into contiguous array
      ================================= */
   if (c_slots != NULL)
     Packc_slots(theEnv,cls,c_slots);
   AddClass(theEnv,cls);
...
static void AddClass(
  ...
  DEFCLASS *cls)
  {
   ...
   if (ctmp != NULL)
     {
      ...
      oldTracec_slots = ctmp->tracec_slots;
#endif
   ...
   /* ==============================================
      Define get- and put- handlers for public c_slots
      ============================================== */
...
/**********************************************************
  ...
  DESCRIPTION  : Forms a contiguous array of instance
                  c_slots for use in creating instances later
                 Also used in determining instance slot
                  indices a priori during handler defns
  INPUTS       : The class
  RETURNS      : Nothing useful
  SIDE EFFECTS : Contiguous array of instance c_slots formed
  NOTES        : None
 **********************************************************/
static void FormInstanceTemplate(
  ...
  DEFCLASS *cls)
  {
   TEMP_SLOT_LINK *ic_slots = NULL,*stmp;
   short scnt = 0;
   long i;

   /* ========================
      Get direct class's c_slots
      ======================== */
   ic_slots = Mergec_slots(theEnv,ic_slots,cls,&scnt,DIRECT);

   /* ===================================================================
      Get all inherited c_slots - a more specific slot takes precedence
   ...
   for (i = 1 ; i < cls->allSuperclasses.classCount ; i++)
     ic_slots = Mergec_slots(theEnv,ic_slots,cls->allSuperclasses.classArray[i],&scnt,INHERIT);

   /* ===================================================
      Allocate a contiguous array to store all the c_slots.
      =================================================== */
...
/**********************************************************
  ...
  SIDE EFFECTS : Contiguous array of integers formed
                 ...
                 also stored to make deletion of the c_slots
                 easier
...
/********************************************************************
  NAME         : Mergec_slots
  DESCRIPTION  : Adds non-duplicate c_slots to list and increments
                   slot count for the class instance template
  INPUTS       : 1) The old slot list
                 2) The address of class containing new c_slots
                 3) Caller's buffer for # of c_slots
                 4) A flag indicating whether the new list of c_slots
 ...
 *******************************************************************/
static TEMP_SLOT_LINK *Mergec_slots(
  void *theEnv,
  TEMP_SLOT_LINK *old,
  DEFCLASS *cls,
  short *scnt,
  int src)
  {
   ...
   /* ======================================
      Process the c_slots in reverse order
      since we are pushing them onto a stack
      ====================================== */
   for (i = (int) (cls->slotCount - 1) ; i >= 0 ; i--)
     {
      newSlot = &cls->c_slots[i];

      /* ==========================================
         A class can prevent it c_slots from being
...
/***********************************************************************
  NAME         : Packc_slots
  DESCRIPTION  : Groups class-c_slots into a contiguous array
                  "c_slots" field points to array
                  "slotCount" field set
  INPUTS       : 1) The class
                 2) The list of c_slots
  RETURNS      : Nothing useful
  SIDE EFFECTS : Temporary list deallocated, contiguous array allocated,
                   and nxt pointers linked
                 Class pointer set for c_slots
  NOTES        : Assumes class->slotCount == 0 && class->c_slots == NULL
 ***********************************************************************/
static void Packc_slots(
  void *theEnv,
  DEFCLASS *cls,
  TEMP_SLOT_LINK *c_slots)
  {
   ...
   cls->c_slots = (SLOT_DESC *) gm2(theEnv,(sizeof(SLOT_DESC) * cls->slotCount));
   stmp = c_slots;
   for (i = 0 ; i < cls->slotCount ; i++)
     {
      sprv = stmp;
      stmp = stmp->nxt;
      GenCopyMemory(SLOT_DESC,1,&(cls->c_slots[i]),sprv->desc);
      cls->c_slots[i].sharedValue.desc = &(cls->c_slots[i]);
      cls->c_slots[i].sharedValue.value = NULL;
