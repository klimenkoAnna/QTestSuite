...
static void GetObjectValueGeneral(
  ...
  struct ObjectMatchVar1 *matchVar)
  {
   ...
   /* =========================================
      We need to reference the basis c_slots if
      ...
      ========================================= */
   if ((theInstance->basisc_slots != NULL) &&
       (! EngineData(theEnv)->JoinOperationInProgress))
     {
      basisSlot = theInstance->basisc_slots + (insSlot - theInstance->slotAddresses);
...
/****************************************************
  ...
  NOTES        : Should only be used to access
                 ...
                 c_slots for pattern and join network
                 comparisons
 ****************************************************/
