...
/*********************************************************
  NAME         : EnvGetDefclassWatchc_slots
  DESCRIPTION  : Determines if changes to c_slots of
                 instances of this class will generate
                 trace messages or not
  INPUTS       : A pointer to the class
  RETURNS      : TRUE if a trace is active,
...
#pragma argsused
#endif
globle unsigned EnvGetDefclassWatchc_slots(
  void *theEnv,
  void *theClass)
  {
#if MAC_MCW || WIN_MCW || MAC_XCD
#pragma unused(theEnv)
   ...
   return(((DEFCLASS *) theClass)->tracec_slots);
  }

/**********************************************************
  NAME         : EnvSetDefclassWatchc_slots
  DESCRIPTION  : Sets the trace to ON/OFF for the
                 changes to c_slots of instances of the class
  INPUTS       : 1) TRUE to set the trace on,
                    FALSE to set it off
                 2) A pointer to the class
  RETURNS      : Nothing useful
...
#pragma argsused
#endif
globle void EnvSetDefclassWatchc_slots(
  void *theEnv,
  unsigned newState,
  void *theClass)
  {
...
#pragma unused(theEnv)
   ...
   ((DEFCLASS *) theClass)->tracec_slots = newState;
  }
  ...
  EXPRESSION *argExprs)
  {
   if (code)
     return(ConstructSetWatchAccess(theEnv,DefclassData(theEnv)->DefclassConstruct,newState,argExprs,
                                    EnvGetDefclassWatchc_slots,EnvSetDefclassWatchc_slots));
   else
     return(ConstructSetWatchAccess(theEnv,DefclassData(theEnv)->DefclassConstruct,newState,argExprs,
