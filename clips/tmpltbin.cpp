...
static void DeallocateDeftemplateBloadData(
  void *theEnv)
  {
   ...
   space =  DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots * sizeof(struct templateSlot);
...
static void BsaveFind(
  void *theEnv)
  {
   ...
   SaveBloadCount(theEnv,DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots);
   SaveBloadCount(theEnv,DeftemplateBinaryData(theEnv)->NumberOfTemplateModules);

   /*==================================================*/
   /* Set the count of deftemplates, deftemplate c_slots */
   /* and deftemplate module data structures to zero.  */
   /*==================================================*/

   DeftemplateBinaryData(theEnv)->NumberOfDeftemplates = 0;
   DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots = 0;
      ...
      for (theDeftemplate = (struct deftemplate *) EnvGetNextDeftemplate(theEnv,NULL);
           ...
           theDeftemplate = (struct deftemplate *) EnvGetNextDeftemplate(theEnv,theDeftemplate))
        {
         ...
         for (theSlot = theDeftemplate->slotList;
           ...
           {
            DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots++;
...
static void BsaveStorage(
  ...
  FILE *fp)
  {
   ...
   GenWrite(&DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots,sizeof(long int),fp);
...
static void BsaveBinaryItem(
  ...
  FILE *fp)
  {
   ...
   space = (DeftemplateBinaryData(theEnv)->NumberOfDeftemplates * sizeof(struct bsaveDeftemplate)) +
           (DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots * sizeof(struct bsaveTemplateSlot)) +
        ...
        theModule = (struct defmodule *) EnvGetNextDefmodule(theEnv,theModule))
     {
      ...
      for (theDeftemplate = (struct deftemplate *) EnvGetNextDeftemplate(theEnv,NULL);
           ...
           theDeftemplate = (struct deftemplate *) EnvGetNextDeftemplate(theEnv,theDeftemplate))
        {
         ...
         tempDeftemplate.numberOfc_slots = theDeftemplate->numberOfc_slots;
         tempDeftemplate.patternNetwork = BsaveFactPatternIndex(theDeftemplate->patternNetwork);

         if (theDeftemplate->slotList != NULL)
           { tempDeftemplate.slotList = DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots; }
         else tempDeftemplate.slotList = -1L;

         GenWrite(&tempDeftemplate,sizeof(struct bsaveDeftemplate),fp);

         DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots += theDeftemplate->numberOfc_slots;
        }
     }
   ...
   /* of deftemplates, deftemplate modules, and deftemplate c_slots */
   ...
   RestoreBloadCount(theEnv,&DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots);
...
static void BloadStorage(
  void *theEnv)
  {
   ...
   GenReadBinary(theEnv,&DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots,sizeof(long int));
   ...
   if (DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots == 0)
     {
      DeftemplateBinaryData(theEnv)->SlotArray = NULL;
      return;
     }
...
static void BloadBinaryItem(
  void *theEnv)
  {
   ...
   BloadandRefresh(theEnv,DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots,sizeof(struct bsaveTemplateSlot),
...
static void UpdateDeftemplate(
  ...
  long obji)
  {
   ...
   theDeftemplate->numberOfc_slots = (unsigned short) bdtPtr->numberOfc_slots;
...
static void ClearBload(
  void *theEnv)
  {
   ...
   for (i = 0; i < DeftemplateBinaryData(theEnv)->NumberOfTemplatec_slots; i++)
