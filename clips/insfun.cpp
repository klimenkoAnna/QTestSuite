...
/******************************************************
  ...
  DESCRIPTION  : Deallocates all the data objects
                 in instance c_slots and then dealloactes
                 the c_slots themeselves
  INPUTS       : The instance
  RETURNS      : Nothing useful
  SIDE EFFECTS : Instance c_slots removed
  ...
  DATA_OBJECT *setVal)
  {
   ...
   if (EngineData(theEnv)->JoinOperationInProgress && sp->desc->reactive &&
       (ins->cls->reactive || sp->desc->shared))
     {
      ...
      EnvPrintRouter(theEnv,WERROR,"Cannot modify reactive instance c_slots while\n");
   ...
   /* =============================================
      ...
      ============================================= */
   if (ins->basisc_slots != NULL)
     {
      spaddr = &ins->slotAddresses[ins->cls->slotNameMap[sp->desc->slotName->id] - 1];
      bsp = ins->basisc_slots + (spaddr - ins->slotAddresses);
      if (bsp->value == NULL)
        {
         bsp->type = sp->type;
         bsp->value = sp->value;
   ...
   if (ins->header.busyCount == 0)
     {
      ...
      if (ins->cls->instanceSlotCount != 0)
        {
         for (i = 0 ; i < ins->cls->instanceSlotCount ; i++)
           if (ins->basisc_slots[i].value != NULL)
             {
              if (ins->basisc_slots[i].desc->multiple)
                MultifieldDeinstall(theEnv,(struct multifield *) ins->basisc_slots[i].value);
              else
                AtomDeinstall(theEnv,(int) ins->basisc_slots[i].type,
                              ins->basisc_slots[i].value);
             }
         rm(theEnv,(void *) ins->basisc_slots,
            (ins->cls->instanceSlotCount * sizeof(INSTANCE_SLOT)));
         ins->basisc_slots = NULL;
        }
     }
  }
   ...
   if (ins->header.busyCount == 0)
     {
      if (ins->cls->instanceSlotCount != 0)
        {
         ins->basisc_slots = (INSTANCE_SLOT *)
                            gm2(theEnv,(sizeof(INSTANCE_SLOT) * ins->cls->instanceSlotCount));
         for (i = 0 ; i < ins->cls->instanceSlotCount ; i++)
           {
            ins->basisc_slots[i].desc = ins->slotAddresses[i]->desc;
            ins->basisc_slots[i].value = NULL;
           }
        }
     }
   ins->header.busyCount++;
