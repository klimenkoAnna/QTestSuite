...
globle SLOT_DESC *FindClassSlot(
  ...
  SYMBOL_HN *sname)
  {
   ...
   for (i = 0 ; i < cls->slotCount ; i++)
     {
      if (cls->c_slots[i].slotName->name == sname)
        return(&cls->c_slots[i]);
     }
   return(NULL);
  }
...
LOCALE void RemoveDefclass(
  ...
  void *vcls)
  {
      ...
      if (cls->c_slots[i].defaultValue != NULL)
        {
         if (cls->c_slots[i].dynamicDefault)
           ReturnPackedExpression(theEnv,(EXPRESSION *) cls->c_slots[i].defaultValue);
         else
           rtn_struct(theEnv,dataObject,cls->c_slots[i].defaultValue);
        }
      DeleteSlotName(theEnv,cls->c_slots[i].slotName);
      RemoveConstraint(theEnv,cls->c_slots[i].constraint);
     }
   ...
   if (cls->slotCount != 0)
     rm(theEnv,(void *) cls->c_slots,(sizeof(SLOT_DESC) * cls->slotCount));
      ...
      if (cls->c_slots[i].defaultValue != NULL)
        {
         ...
         if (cls->c_slots[i].dynamicDefault)
           ...
           rtn_struct(theEnv,dataObject,cls->c_slots[i].defaultValue);
#else
         if (cls->c_slots[i].dynamicDefault == 0)
           rtn_struct(theEnv,dataObject,cls->c_slots[i].defaultValue);
#endif
        }
     }
...
/***************************************************
  ...
  NOTES        : A slot name always has the same
                 ...
                 for immediate lookup of c_slots
