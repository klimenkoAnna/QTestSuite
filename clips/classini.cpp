        ...
        for (cls = DefclassData(theEnv)->ClassTable[j] ; cls != NULL ; cls = cls->nxtHash)
          {
           for (i = 0 ; i < cls->slotCount ; i++)
             {
              if ((cls->c_slots[i].defaultValue != NULL) && (cls->c_slots[i].dynamicDefault == 0))
                {
                 tmpexp = ((DATA_OBJECT *) cls->c_slots[i].defaultValue)->supplementalInfo;
                 rtn_struct(theEnv,dataObject,cls->c_slots[i].defaultValue);
                 cls->c_slots[i].defaultValue = tmpexp;
                }
             }
          }
     }
           ...
           for (i = 0 ; i < cls->slotCount ; i++)
             {
              ...
              if ((cls->c_slots[i].defaultValue != NULL) && (cls->c_slots[i].dynamicDefault == 0))
                {
                 tmpexp = ((DATA_OBJECT *) cls->c_slots[i].defaultValue)->supplementalInfo;
                 ValueDeinstall(theEnv,(DATA_OBJECT *) cls->c_slots[i].defaultValue);
                 rtn_struct(theEnv,dataObject,cls->c_slots[i].defaultValue);
                 cls->c_slots[i].defaultValue = tmpexp;
                }
             }
      ...
      for (i = 0 ; i < cls->slotCount ; i++)
        {
         if ((cls->c_slots[i].defaultValue != NULL) && (cls->c_slots[i].dynamicDefault == 0))
           {
            tmpexp = cls->c_slots[i].defaultValue;
            cls->c_slots[i].defaultValue = (void *) get_struct(theEnv,dataObject);
            EvaluateAndStoreInDataObject(theEnv,(int) cls->c_slots[i].multiple,(EXPRESSION *) tmpexp,
                                         (DATA_OBJECT *) cls->c_slots[i].defaultValue,TRUE);
            ValueInstall(theEnv,(DATA_OBJECT *) cls->c_slots[i].defaultValue);
            ((DATA_OBJECT *) cls->c_slots[i].defaultValue)->supplementalInfo = tmpexp;
           }
        }
     }
...
static void SetupDefclasses(
  void *theEnv)
  {
...
#endif
   EnvDefineFunction2(theEnv,"class-c_slots",'m',PTIEF Classc_slotsCommand,"Classc_slotsCommand","12w");
   ...
   EnvDefineFunction2(theEnv,"slot-sources",'m',PTIEF c_slotsourcesCommand,"c_slotsourcesCommand","22w");
   ...
   AddWatchItem(theEnv,"c_slots",1,&DefclassData(theEnv)->Watchc_slots,74,DefclassWatchAccess,DefclassWatchPrint);
#endif
  }
