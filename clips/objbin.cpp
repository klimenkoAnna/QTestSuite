...
static void Bsavec_slots(void *,struct constructHeader *,void *);
static void BsaveTemplatec_slots(void *,struct constructHeader *,void *);
...
#pragma unused(buf)
   ...
   for (i = 0 ; i < cls->slotCount ; i++)
     {
      cls->c_slots[i].bsaveIndex = ObjectBinaryData(theEnv)->SlotCount++;
      cls->c_slots[i].overrideMessage->neededSymbol = TRUE;
      if (cls->c_slots[i].defaultValue != NULL)
        {
         if (cls->c_slots[i].dynamicDefault)
           {
            ExpressionData(theEnv)->ExpressionCount +=
              ExpressionSize((EXPRESSION *) cls->c_slots[i].defaultValue);
            MarkNeededItems(theEnv,(EXPRESSION *) cls->c_slots[i].defaultValue);
           }
         else
           {
            /* =================================================
               ...
               ================================================= */
            tmpexp =
              ConvertValueToExpression(theEnv,(DATA_OBJECT *) cls->c_slots[i].defaultValue);
   ...
   /* ========================================
      Count canonical c_slots needed by defclass
      ======================================== */
   ...
   for (i = 0 ; i < cls->slotCount ; i++)
     {
      if (cls->c_slots[i].defaultValue != NULL)
        {
         if (cls->c_slots[i].dynamicDefault)
           BsaveExpression(theEnv,(EXPRESSION *) cls->c_slots[i].defaultValue,(FILE *) buf);
         else
           {
            /* =================================================
               Static default values are stotred as data objects
...
/*************************************************************************************
  ...
  DESCRIPTION  : Writes out number of each type of structure required for COOL
                 ...
                 Number of c_slots (long)
                 Number of instance template c_slots (long)
...
static void BsaveObjects(
  ...
  FILE *fp)
  {
   ...
   /* ===================
      Write out the c_slots
      =================== */
   DoForAllConstructs(theEnv,Bsavec_slots,DefclassData(theEnv)->DefclassModuleIndex,FALSE,(void *) fp);

   /* =====================================
      Write out the template instance c_slots
      ===================================== */
   DoForAllConstructs(theEnv,BsaveTemplatec_slots,DefclassData(theEnv)->DefclassModuleIndex,FALSE,(void *) fp);
...
static void BsaveDefclass(
  ...
  void *buf)
  {
   ...
   if (cls->c_slots != NULL)
     {
      dummy_class.c_slots = ObjectBinaryData(theEnv)->SlotCount;
      ObjectBinaryData(theEnv)->SlotCount += (long) cls->slotCount;
     }
   else
     dummy_class.c_slots = -1L;
...
/***************************************************
  NAME         : Bsavec_slots
  DESCRIPTION  : Writes class c_slots binary data
  INPUTS       : 1) The defclass
                 2) The binary file pointer
  RETURNS      : Nothing useful
  SIDE EFFECTS : Defclass c_slots binary data written
  NOTES        : None
 ***************************************************/
static void Bsavec_slots(
  void *theEnv,
  struct constructHeader *theDefclass,
  void *buf)
  {
...
#pragma argsused
#endif
static void BsaveTemplatec_slots(
  void *theEnv,
  struct constructHeader *theDefclass,
  void *buf)
  {
...
static void UpdateDefclass(
  ...
  long obji)
  {
   ...
   cls->tracec_slots = DefclassData(theEnv)->Watchc_slots;
#endif
   ...
   cls->c_slots = SlotPointer(bcls->c_slots);
